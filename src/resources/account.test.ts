// src/resources/account.test.ts

import { describe, it, expect, vi, afterEach } from "vitest";
import { TinySDK } from "../index.js";
import { TinyApiError } from "../errors/tiny-api-error.js";
import type { InfoSuccessResponse, AccountDetails } from "../types/account.ts";
...
const mockAccountSuccessResponse: InfoSuccessResponse = {
...
describe("Account Resource (sdk.account)", () => {
  it("deve retornar os dados da conta em caso de sucesso", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ retorno: mockAccountSuccessResponse }), {
        status: 200,
      })
    );
    ...
    expect(result).toEqual(mockAccountSuccessResponse.conta);
